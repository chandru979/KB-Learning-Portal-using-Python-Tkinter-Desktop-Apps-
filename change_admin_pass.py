...
class change_admin_password():
    def __init__(self, root):
        self.root = root
        self.root.geometry('1920x1080')
        self.root.title('change admin password')
        change_admin_password.forgot_pass(self)
    ...
    def forgot_pass(self):
        ...
        forgot_label = customtkinter.CTkLabel(frame1, text="Forgot/Change admin Password", text_font=("Times new roman", 15, "bold"),text_color='black',bg_color="#263238", fg_color="orange",width=419)
    ...
    def verify(self,email):
        ...
        my_cursor.execute("select count(*) from admin where admin_email=%s", (email,))
    ...
    def changepassword(self,email,password,confirm_password):
        ...
        if(password == confirm_password):
            connection = mysql.connector.connect(user='root', password='password', host='127.0.0.1',database='kbportal', port='3306')
            my_cursor = connection.cursor()
            my_cursor.execute("update admin set admin_password=%s where admin_email=%s", (password,email))
            connection.commit()
            connection.close()
...
if __name__ == '__main__':
    root=Tk()
    obj=change_admin_password(root)
    root.mainloop()